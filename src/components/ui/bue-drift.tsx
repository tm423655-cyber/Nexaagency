...
//
// Cores ajustadas para a paleta da Nexa Agency (ver src/app/globals.css):
// void #01040B -> abyss #060C1A -> brand-deep #0736A8 -> brand #0B5BFF -> cyan #22D9FF.
...
// Paleta da Nexa (ver --color-void/abyss/brand-deep/brand/cyan em globals.css),
// do mais escuro ao mais claro, para o mesh derivar em azul/ciano sobre o fundo
// quase preto do site em vez do azul-petroleo original gerado pelo 21st.dev.
const UNIFORMS = {
  colors: [
    [0.00392156862745098, 0.01568627450980392, 0.043137254901960784], // void #01040B
    [0.023529411764705882, 0.047058823529411764, 0.10196078431372549], // abyss #060C1A
    [0.027450980392156862, 0.21176470588235294, 0.6588235294117647], // brand-deep #0736A8
    [0.043137254901960784, 0.35686274509803922, 1], // brand #0B5BFF
    [0.13333333333333333, 0.8509803921568627, 1], // cyan #22D9FF
    [0.13333333333333333, 0.8509803921568627, 1],
    [0.13333333333333333, 0.8509803921568627, 1],
    [0.13333333333333333, 0.8509803921568627, 1],
...
// Compilar o shader e abrir o contexto WebGL custa tempo de thread
// principal (especialmente em celulares com CPU/GPU mais fracas). Adiar
// esse trabalho para quando o navegador estiver ocioso evita que ele
// compita com a hidratacao e com o carregamento inicial da pagina — o
// mesh some por uma fracao de segundo a mais, mas o site fica responsivo
// mais cedo. Sem requestIdleCallback (Safari), um pequeno setTimeout cai
// pra fora da tarefa atual, o que ja ajuda.
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    const cancelIdle = onIdle(() => {
      if (!cancelled) teardown = setup(canvas, reduceMotion)
    })
...
function setup(canvas: HTMLCanvasElement, reduceMotion: boolean): (() => void) | undefined {
  ...
  // O mesh so anima quando ha tempo/deriva configurados e a pessoa nao
  // pediu menos movimento — respeita prefers-reduced-motion e evita o
  // loop de requestAnimationFrame rodando pra sempre sem necessidade.
  const timeAnimated = Math.abs(UNIFORMS.timeScale) > 0.0001 && !reduceMotion
  ...
  // Rolagem dispara com muita frequencia — adia a leitura de layout
  // (getBoundingClientRect forca um reflow sincrono) para o proximo
  // frame em vez de rodar a cada evento de scroll.
  const onScroll = () => {
  ...
  function render(now: number) {
    ...
    resizeCanvas()
    // TypeScript doesn't carry the `if (!gl) return` narrowing above into
    // this nested function declaration — assert it back, as before.
